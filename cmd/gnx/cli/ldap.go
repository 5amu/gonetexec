...
func NewLDAPCmd() *cobra.Command {
	...
	queryFlags := pflag.NewFlagSet("Query Flags", pflag.ContinueOnError)
	queryFlags.StringVarP(&customFilter, "filter", "f", "", "Bring your own filter")
	queryFlags.StringVarP(&customAttributes, "attributes", "a", "", "Ask your attributes (comma separated)")
	queryFlags.BoolVar(&adminCount, "admin-count", false, "Enumerate objects that have an adminCount")
	queryFlags.BoolVar(&computers, "computers", false, "Enumerate objects that are computers")
	queryFlags.BoolVar(&groups, "groups", false, "Enumerate objects that are domain groups")
	queryFlags.BoolVar(&users, "users", false, "Enumerate objects that are enabled domain users")
	queryFlags.BoolVar(&activeUsers, "active-users", false, "Enumerate objects that are active enabled domain users")
	...
	filterFlags.BoolVar(&disabled, "disabled", false, "Filter for objects with flag ACCOUNTDISABLE")
	filterFlags.BoolVar(&homedirRequired, "homedir-required", false, "Filter for objects with flag HOMEDIR_REQUIRED")
	filterFlags.BoolVar(&lockout, "lockout", false, "Filter for objects with flag LOCKOUT")
	filterFlags.BoolVar(&pwdNotReqd, "password-not-required", false, "Filter for objects with flag PASSWD_NOTREQD")
	filterFlags.BoolVar(&pwdCantChange, "password-cant-change", false, "Filter for objects with flag PASSWD_CANT_CHANGE")
	filterFlags.BoolVar(&encTextPwd, "encrypted-text-pwd-allowed", false, "Filter for objects with flag ENCRYPTED_TEXT_PWD_ALLOWED")
	filterFlags.BoolVar(&tempDupAccount, "temp-duplicate-account", false, "Filter for objects with flag TEMP_DUPLICATE_ACCOUNT")
	filterFlags.BoolVar(&normalAccount, "normal-account", false, "Filter for objects with flag NORMAL_ACCOUNT")
	filterFlags.BoolVar(&interdomainTrust, "interdomain-trust-account", false, "Filter for objects with flag INTERDOMAIN_TRUST_ACCOUNT")
	filterFlags.BoolVar(&workstationTrust, "workstation-trust-account", false, "Filter for objects with flag WORKSTATION_TRUST_ACCOUNT")
	filterFlags.BoolVar(&serverTrust, "server-trust-account", false, "Filter for objects with flag SERVER_TRUST_ACCOUNT")
	filterFlags.BoolVar(&dontExpirePwd, "password-never-expires", false, "Filter for objects with flag DONT_EXPIRE_PASSWD")
	filterFlags.BoolVar(&mnsLogon, "mns-logon-account", false, "Filter for objects with flag MNS_LOGON_ACCOUNT")
	filterFlags.BoolVar(&smartcardReq, "smartcard-required", false, "Filter for objects with flag SMARTCARD_REQUIRED")
	filterFlags.BoolVar(&trustedDeleg, "trusted-for-delegation", false, "Filter for objects with flag TRUSTED_FOR_DELEGATION")
	filterFlags.BoolVar(&notDelegated, "not-delegated", false, "Filter for objects with flag NOT_DELEGATED")
	filterFlags.BoolVar(&useDES, "use-des-key-only", false, "Filter for objects with flag USE_DES_KEY_ONLY")
	filterFlags.BoolVar(&dontRequirePre, "dont-require-preauth", false, "Filter for objects with flag DONT_REQ_PREAUTH")
	filterFlags.BoolVar(&pwdExpired, "password-expired", false, "Filter for objects with flag PASSWORD_EXPIRED")
	filterFlags.BoolVar(&trustedToAuth, "trusted-to-auth-for-delegation", false, "Filter for objects with flag TRUSTED_TO_AUTH_FOR_DELEGATION")
	filterFlags.BoolVar(&partialSecrets, "partial-secrets-account", false, "Filter for objects with flag PARTIAL_SECRETS_ACCOUNT")
	filterFlags.BoolVar(&readNot, "not", false, "Negate next filter")
	...
	return generateCli(cmd, connFlags, hashFlags, createFlags, queryFlags, filterFlags, deleteFlags)
}
